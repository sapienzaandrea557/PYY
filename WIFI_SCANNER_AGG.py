...
# Silenzia i warning di Scapy
logging.getLogger("scapy.runtime").setLevel(logging.ERROR)

try:
    from scapy.all import ARP, Ether, srp, conf, IP, ICMP, sr1, TCP, sendp, get_if_hwaddr, sniff, DNSRR, DNSQR, DNS, UDP
    # Impostazione globale per silenziare i warning di Scapy
    conf.verb = 0
except ImportError:
    pass
...
# Database OUI esteso per identificare Produttore e Tipo Dispositivo
OUI_DB = {
    "00:0C:29": ("VMware", "Virtual Machine"),
    "08:00:27": ("VirtualBox", "Virtual Machine"),
    "00:15:5D": ("Microsoft", "Virtual Machine (Hyper-V)"),
    "00:1C:42": ("Parallels", "Virtual Machine"),
    "00:03:93": ("Apple", "Device"),
    "00:1E:52": ("Apple", "Device"),
    "10:38:1F": ("Apple", "Device"),
    "D8:CF:9C": ("Apple", "Device"),
    "FC:FC:48": ("Apple", "Device"),
    "4C:E0:DB": ("Samsung", "Phone/Tablet"),
    "24:FC:E5": ("Samsung", "Phone/Tablet"),
    "A8:7D:12": ("Samsung", "Phone/Tablet"),
    "00:E0:4C": ("Realtek", "PC/Laptop"),
    "B8:27:EB": ("Raspberry Pi", "SBC"),
    "DC:A6:32": ("Raspberry Pi", "SBC"),
    "E4:5F:01": ("Raspberry Pi", "SBC"),
    "00:1A:11": ("Google", "Android/Chromecast"),
    "38:F3:AB": ("Xiaomi", "Phone/Tablet"),
    "64:9E:F3": ("Huawei", "Phone/Tablet"),
    "A4:2A:95": ("TP-Link", "Router"),
    "98:00:6A": ("Technicolor", "Router"),
    "00:14:D1": ("Trendnet", "Router"),
    "00:24:D4": ("Netgear", "Router"),
    "C0:C5:20": ("D-Link", "Router"),
    "00:22:F4": ("Samsung TV", "Smart TV"),
    "70:2A:D5": ("LG", "Smart TV"),
    "D4:6A:91": ("Amazon", "Echo/Kindle"),
    "00:17:88": ("Philips Hue", "Smart Light"),
    "68:37:E9": ("Amazon", "Echo/Kindle"),
    "50:DC:E7": ("Apple", "Apple TV"),
    "00:11:32": ("Synology", "NAS"),
}

def get_vendor_and_type(mac):
    if not mac or mac == "—": return "Unknown", "Unknown"
    prefix = mac.upper().replace("-", ":")[:8]
    return OUI_DB.get(prefix, ("Unknown", "Unknown"))

def get_hostname_advanced(ip):
    # Metodo 1: DNS Inverso
    try:
    ...
    # Metodo 2: NetBIOS (porta 137)
    try:
        nb_query = b"\x80\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x20CKAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\x00\x00\x21\x00\x01"
...
def detect_os(ip, mac=""):
    """Rileva OS, versione e tipo dispositivo in modo approfondito"""
    os_name = "Unknown"
    os_ver = ""
    dev_type = "Unknown"
    
    vendor, d_type = get_vendor_and_type(mac)
    if d_type != "Unknown":
        dev_type = d_type

    # 1. Scansione porte rapida
    open_ports = []
    # 22:SSH, 80:HTTP, 135:RPC, 137:NetBIOS, 443:HTTPS, 445:SMB, 3389:RDP, 554:RTSP, 5000:UPnP, 5555:ADB, 8008:Cast, 62078:iOS
    scan_ports = [22, 80, 135, 137, 139, 443, 445, 554, 3389, 5000, 5555, 8008, 62078]
    ...
    ttl = None
    try:
        out = subprocess.run(["ping", "-n", "1", "-w", "400", ip], 
    ...
    if ttl:
        if ttl <= 64:
            ...
            if 62078 in open_ports or "Apple" in vendor:
                os_name = "iOS/macOS"
                dev_type = "Apple Device"
            elif 5555 in open_ports or 8008 in open_ports:
                os_name = "Android"
                dev_type = "Phone/TV"
                ...
                try:
                    ...
                    if "Ubuntu" in banner: 
                        os_name = "Ubuntu"
                        os_ver = banner.split("-")[0].replace("SSH-2.0-OpenSSH_8.2p1 ", "")
                    elif "Debian" in banner: os_name = "Debian"
                    elif "Kali" in banner: os_name = "Kali Linux"
                except: pass
        elif ttl <= 128:
            os_name = "Windows"
            dev_type = "PC/Laptop"
            if 445 in open_ports: os_ver = "10/11"
            if 3389 in open_ports: os_ver += " (Pro/Ent)"
            if 135 in open_ports and not os_ver: os_ver = "Workstation"
        else:
            os_name = "Network Device"
            dev_type = "Router/Switch"

    if 8008 in open_ports: dev_type = "Chromecast/Google Home"
    if 554 in open_ports: dev_type = "IP Camera / DVR"
    if 137 in open_ports or 139 in open_ports: 
        if os_name == "Unknown": os_name = "Windows/Samba"
    
    # Se il vendor è Technicolor o TP-Link e l'IP finisce per .1, è quasi certamente un Router
    if ip.endswith(".1") and ("Technicolor" in vendor or "TP-Link" in vendor or "Gateway" in vendor):
        dev_type = "Router/Gateway"
        os_name = "Embedded Linux"
    elif ip.endswith(".1") and dev_type == "Unknown":
        dev_type = "Router"

    final_os = f"{os_name} {os_ver}".strip()
    if dev_type != "Unknown":
        return f"{final_os} ({dev_type})"
    return final_os
...
class WifiScanner(tk.Tk):
    ...
    def _build_ui(self):
        ...
        cols = ("ip", "hostname", "os", "mac", "stato", "blocco")
...
<p>Inserisci le tue credenziali per accedere alla rete</p>
    ...
    def _arp_scan(self, subnet):
        ...
        # 1. ARP SCAN (Scapy) - Molto veloce e preciso
        try:
            ...
            # Fallback a ping se scapy fallisce (es. permessi)
            def ping(ip):
        ...
        for ip, d in devices.items():
            ...
            # OS e Tipo Dispositivo
            os_info = detect_os(ip, d["mac"])
            ...
            results.append({
                ...
                "blocco": "●" if is_blocked else "—", 
                "tag": tag
            })

        # Aggiungi questo PC se non presente
        if my_ip and my_ip not in devices:
            try: hostname = socket.gethostname()
            except: hostname = "questo PC"
            results.append({
                "ip": my_ip, "hostname": hostname, "mac": "—",
                "os": f"Windows ({platform.release()}) [TU]",
    ...
    def _scan_done(self, devices, error=None):
        ...
        for d in devices:
            self.tree.insert("", "end",
                             values=(d["ip"], d["hostname"], d["os"], d["mac"], d["stato"], d["blocco"]),
                             tags=(d["tag"],))
        n = len(devices)
